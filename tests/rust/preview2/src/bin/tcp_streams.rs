use wasi_tests::tcp_helpers::{ blocking_accept, blocking_bind, blocking_connect, blocking_listen,
    blocking_read_to_end, blocking_write_util, ip_socket_address_new,
    new_loopback, tcp_socket_new };
...
fn test_tcp_input_stream_should_be_closed_by_local_shutdown(
    ...
    setup(net, family, |server, client| {
        ...
        blocking_write_util(&server.output, b"Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.").unwrap();
...
fn test_tcp_shutdown_should_not_lose_data(net: &Network, family: IpAddressFamily) {
    setup(net, family, |server, client| {
        ...
        let incoming_data = blocking_read_to_end(&server.input).unwrap();
        assert_eq!(
            outgoing_data.len(),
            incoming_data.len(),
...
fn setup(net: &Network, family: IpAddressFamily, body: impl FnOnce(Connection, Connection)) {
    ...
    let client_socket = tcp_socket_new(family).unwrap();
    let (client_input, client_output) = blocking_connect(&client_socket, net, bound_address).unwrap();
    let (accepted_socket, accepted_input, accepted_output) = blocking_accept(&listener).unwrap();
