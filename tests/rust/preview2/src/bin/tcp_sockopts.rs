...
fn test_tcp_sockopt_inheritance(net: &Network, family: IpAddressFamily) {
    let bind_addr = ip_socket_address_new(new_loopback(family), 0);
    ...
    blocking_bind(&listener, &net, bind_addr).unwrap();
    blocking_listen(&listener).unwrap();
    let bound_addr = listener.local_address().unwrap();
    let client = tcp_socket_new(family).unwrap();
    blocking_connect(&client, &net, bound_addr).unwrap();
    let (accepted_client, _, _) = blocking_accept(&listener).unwrap();
